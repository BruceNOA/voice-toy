...
#define GPIO_CHARGE_PORT     GPIO_PORT_0
#define GPIO_CHARGE_PIN      GPIO_PIN_2

#define GPIO_PG_PORT     GPIO_PORT_0
#define GPIO_PG_PIN      GPIO_PIN_3
...
typedef struct 
{
	char device_power_state;
//	char device_pairing_state;
	char device_connected_state;
	char device_paired_flg;
	char device_charge_state;
	char device_charger_state;
	char device_battery_state;
}_DEVICE_STATE;

typedef struct 
{
	unsigned char usage_data_write[7];
	unsigned char usage_data_to_read[7];
	u16 usage_data_count_write;
	u16 usage_data_count_read;
	uint32_t usage_data_write_address;
	uint32_t usage_data_read_address;
	
	unsigned int usage_data_read_address_cnt;
	
}_USAGE_DATA;

extern _USAGE_DATA _usage_data_;

#define DEVICE_WORK_TIME_MAX 480//480
#define DEVICE_COUNT_DOWN_TIME_MAX 470//470
#define DEVICE_COUNT_DOWN_TIME (DEVICE_WORK_TIME_MAX - DEVICE_COUNT_DOWN_TIME_MAX)

#define DEVICE_PAIRING_TIME_MAX 180//180 

#define APP_USER_TIMER_DELAY 5
extern ke_msg_id_t timer_user;
extern void app_user_timer_cb_handler(void);

void uart2_print_string(const char *str);

void LED_Power_Control(char cmd);
...
extern _DEVICE_STATE device_state;

extern uint8_t central_bd_addr[BD_ADDR_LEN];
extern uint8_t central_bd_addr_type;
